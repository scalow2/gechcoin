...
#if defined(NDEBUG)
# error "GECH cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "GECH Signed Message:\n";
...
{
    int64_t tmpSpacing = 0;
    tmpSpacing = (pindexBestHeader->nHeight < consensusParams.nHarkFork1) ? consensusParams.nPowTargetSpacing : consensusParams.nPowTargetSpacing2;

    return chainActive.Tip()->GetBlockTime() > GetAdjustedTime() - tmpSpacing * 20;
}
                              ...
                              std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s GECH additional fees, %d delta bytes\n",
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    else if (strCommand == NetMsgType::GETBLOCKS)
    {
        ...
        for (; pindex; pindex = chainActive.Next(pindex))
        {
            ...
            int64_t tmpSpacing = 0;
            tmpSpacing = (pindex->nHeight < chainparams.GetConsensus().nHarkFork1) ? chainparams.GetConsensus().nPowTargetSpacing : chainparams.GetConsensus().nPowTargetSpacing2;

            const int nPrunedBlocksLikelyToHave = MIN_BLOCKS_TO_KEEP - 3600 / tmpSpacing;
...
bool SendMessages(CNode* pto)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        if (!pto->fDisconnect && state.vBlocksInFlight.size() > 0) {
            ...
            int64_t tmpSpacing = 0;
            tmpSpacing = (queuedBlock.pindex->nHeight < consensusParams.nHarkFork1) ? consensusParams.nPowTargetSpacing : consensusParams.nPowTargetSpacing2;
            if (nNow > state.nDownloadingSince + tmpSpacing * (BLOCK_DOWNLOAD_TIMEOUT_BASE + BLOCK_DOWNLOAD_TIMEOUT_PER_PEER * nOtherPeersWithValidatedDownloads)) {
