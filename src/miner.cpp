...
static bool ProcessBlockFound(const CBlock* pblock, const CChainParams& chainparams)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("GECHMiner: generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, chainparams, NULL, pblock, true, NULL))
        return error("GECHMiner: ProcessNewBlock, block not accepted");
...
void static BitcoinMiner(const CChainParams& chainparams)
{
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("Error in GECHMiner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("Running GECHMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
            ...
            while (true) {
                ...
                if (ScanHash(pblock, nNonce, &hash))
                {
                    ...
                    {
                        LogPrintf("GECHMiner nonce matches!\n");
                        // Found a solution
    ...
    {
        LogPrintf("GECHMiner terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("GECHMiner runtime error: %s\n", e.what());
        return;
    }
}
